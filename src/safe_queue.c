...
{
    ElementDestroy m_func; /* <Destroy function use ForEach function for destroy all element> */
}Destroy;

static SQueue* InitSQueue(SQueue* _squeue, size_t _initSize);
static SQueue* InitSemMutex(SQueue* _squeue,size_t _initSize);
static int MyActionDestroy(void* _element, void* _context);
static void* GetElement(SQueue* _squeue);
static void PutElement(SQueue* _squeue, void* _element);
...
SQueue* SQueue_Create(size_t _initSize)
{
    ...
    return InitSQueue(pQue,_initSize);
}
...
void SQueue_Destroy(SQueue** _squeue,ElementDestroy _desFunc)
{
    ...
    if(_squeue != NULL && *_squeue != NULL)
    {
		...
		{
            des.m_func = _desFunc;
            SQueue_ForEach(*_squeue, MyActionDestroy, &des);
		}
...
SQueue_Result SQueue_Remove(SQueue* _squeue,void** _returnElement)
{
    ...
    *_returnElement = GetElement(_squeue);
    return 	SQUEUE_SUCCESS;
}
...
static SQueue* InitSQueue(SQueue* _squeue, size_t _initSize)
{
    ...
    _squeue =  InitSemMutex(_squeue,_initSize);
    if(_squeue == NULL)
    {
        return NULL;
    }
