...
static HashMap* InitHashMap(HashMap* _hash,List** _plists,size_t _capacity, HashFunction _hashFunc, EqualityFunction _keysEqualFunc);
static void InitMap_Stats(Map_Stats* _stats,size_t _capacity);

static size_t FindNextPrime(size_t _capacity);
static int PrimeNumCheck(size_t _n);

static Elements* CreateNewPair(const void* _key,const void* _value);
static ListItr CalcIndexSearchKey(const HashMap* _map,const void* _key);
static int SearchKey(void* _item, void* _context);

static int ActionOnPair(void* _item, void* _context);

static void DestroyList(List* _list,void (*_keyDestroy)(void* _key), void (*_valDestroy)(void* _value));

static void CheckMax(size_t* _maxChainLength, size_t _listSize);
...
HashMap* HashMap_Create(size_t _capacity, HashFunction _hashFunc, EqualityFunction _keysEqualFunc)
{
    ...
    return InitHashMap(hash,plists, _capacity,_hashFunc, _keysEqualFunc);
}
...
void HashMap_Destroy(HashMap** _map, void (*_keyDestroy)(void* _key), void (*_valDestroy)(void* _value))
{
    ...
    {
        DestroyList((*_map)->m_lists[i-1],_keyDestroy, _valDestroy);
...
Map_Result HashMap_Insert(HashMap* _map, const void* _key, const void* _value)
{
    ...
    itr = CalcIndexSearchKey(_map, _key);
    if(itr != ListItr_Next(itr))
    {
        return MAP_KEY_DUPLICATE_ERROR;
    }
    
    elements = CreateNewPair(_key, _value);
    if(elements == NULL)
    {
        return MAP_ALLOCATION_ERROR;
    }
...
Map_Result HashMap_Remove(HashMap* _map, const void* _searchKey, void** _pKey, void** _pValue)
{
    ...
    itr = CalcIndexSearchKey(_map, _searchKey);
    if(itr == ListItr_Next(itr))
    {
...
Map_Result HashMap_Find(const HashMap* _map, const void* __searchKey, void** _pValue)
{
    ...
    itr = CalcIndexSearchKey(_map, __searchKey);
...
size_t HashMap_ForEach(const HashMap* _map, KeyValueActionFunction _action, void* _context)
{
    ...
    for(idx = 0 ; idx < _map->m_capacity ; ++idx)
    {
        ...
        itr = ListItr_ForEach(begin,end, ActionOnPair, &sForEach);
        if(end != itr)
        {
            return idx;
        }
...
Map_Stats HashMap_GetStatistics(const HashMap* _map)
{
    ...
    InitMap_Stats(&stats,_map->m_capacity);
    for(idx = 0 ; idx < _map->m_capacity ; ++idx)
    {
        begin = ListItr_Begin(_map->m_lists[idx]);
        end = ListItr_End(_map->m_lists[idx]);
        if(end != begin)
        {
            ...
            CheckMax(&(stats.maxChainLength), tempListSize);
...
static int ActionOnPair(void* _item, void* _context)
{
     ...
     return sForEach->m_KeyValFunc(element->m_key,element->m_value,sForEach->m_context);
}
...
static ListItr CalcIndexSearchKey(const HashMap* _map,const void* _key)
{
    ...
    itr = ListItr_FindFirst(begin,end, SearchKey, &search);
...
static size_t FindNextPrime(size_t _capacity)
{
    while(PrimeNumCheck(_capacity) != 0)
    {
        ++_capacity;
    }
    return _capacity;
